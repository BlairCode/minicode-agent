...
- **文件化输入与可见输出**：可上传 UTF-8 文本或源码；模型初始只接收文件名并按需读取。网页展示步骤摘要、工具结果、Markdown 回答和文件预览，也可打开文件所在目录；长对话在中央区域独立滚动，输入区始终保留在视口内。
...
如果 PowerShell 禁止执行激活脚本，也可以直接使用虚拟环境解释器：
...
修改代码后应先结束旧进程再重新启动。若默认端口仍被占用，程序会打印警告并改用一个临时端口；此时以终端输出或自动打开的地址为准，不要继续使用旧的 `8765` 页面。网页也会检查后台接口版本，发现新旧进程不一致时会要求重启。
...
当前 75 项离线测试覆盖 Agent Loop、首轮工作区提示、可取消重试、连续错误恢复、上下文裁剪、Qwen Tool Calling、上传校验与隔离、前后端能力协商、端口独占、工具 Schema 边界、文件夹定位、工作区可执行文件解析、命令策略、审批、超时、多轮对话、本地 HTTP 安全头和页面高度约束。
